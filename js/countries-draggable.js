document.addEventListener('DOMContentLoaded', () => {
  ...
  let dragged = null;
  window.currentOrder = []; // сюда записывается текущая последовательность

  // Функция для обновления порядка элементов
  function updateOrder() {
    window.currentOrder = Array.from(container.querySelectorAll('[draggable]'))
      .map(el => el.dataset.sort);
    console.log('Текущий порядок:', window.currentOrder);
